...
from src.underia import game, styles, inventory, weapons, entity, projectiles
from src.values import hp_system, damages, effects
from src.resources import time, position, cursors, constants
import pygame as pg
import random
...
class Player:
    ...
    def get_screen_scale(self):
        ACCESSORY_SIZE = {'aimer': 1.5, 'winds_necklace': 1.1}
        t = 1.0
        for i in range(len(self.accessories)):
    ...
    def update(self):
        ...
        if len([1 for eff in self.hp_sys.effects if eff.NAME == 'Shield']):
        ...
        elif inventory.TAGS['bow'] in inventory.ITEMS[w.name.replace(' ', '_')].tags or inventory.TAGS['gun'] in inventory.ITEMS[w.name.replace(' ', '_')].tags:
            if pg.K_y in game.get_game().get_keys():
                if 'windstorm_assassin_mark' in self.accessories:
                    if self.mana >= 60:
                        self.mana -= 60
                        mx, my = position.real_position(displayer.reflect(*pg.mouse.get_pos()))
                        self.obj.apply_force(vector.Vector(vector.coordinate_rotation(mx - self.obj.pos[0], my - self.obj.pos[1]), 3000))
                    ...
                    if self.mana >= 60:
                        ...
                        mx, my = position.real_position(displayer.reflect(*pg.mouse.get_pos()))
                        for i in range(40):
                            x, y = mx + random.randint(-100 - i * 20, 100 + i * 20), my - game.get_game().displayer.SCREEN_HEIGHT // 2 - i * 12
                            pj = ammo((x, y), vector.coordinate_rotation(mx - x, my - y) + random.randint(-2, 2), 600, 1200)
                            game.get_game().projectiles.append(pj)
        else:
            if pg.K_y in game.get_game().get_keys():
                if 'windstorm_swordman_mark' in self.accessories:
                    if not w.cool and self.mana >= 80:
                        ...
                        for e in game.get_game().entities:
                            ...
                            if vector.distance(ax, ay) < 500:
                                e.hp_sys.damage(w.damages[damages.DamageTypes.PHYSICAL] * 3, damages.DamageTypes.PHYSICAL)
                    ...
                    if not w.cool and self.mana >= 40:
                        ...
                        for e in game.get_game().entities:
                            ...
                            ay = e.obj.pos[1] - py
                            if vector.distance(ax, ay) < 500:
                                e.hp_sys.damage(w.damages[damages.DamageTypes.PHYSICAL] * 10, damages.DamageTypes.PHYSICAL)
    ...
    def ui(self):
        ...
        if self.open_inventory:
            ...
            if len(self.recipes):
                ...
                styles.item_display(game.get_game().displayer.SCREEN_WIDTH - 260, game.get_game().displayer.SCREEN_HEIGHT - 170,
                                    cur_recipe.result, str(self.sel_recipe + 1), str(cur_recipe.crafted_amount), 2)
                i = 0
                for item, amount in cur_recipe.material.items():
                    styles.item_display(game.get_game().displayer.SCREEN_WIDTH - 20 - 80 * (1 + len(cur_recipe.material)) + 80 * i,
                                        game.get_game().displayer.SCREEN_HEIGHT - 250, item, '', str(amount), 1)
                    i += 1
                if pg.Rect(game.get_game().displayer.SCREEN_WIDTH - 260, game.get_game().displayer.SCREEN_HEIGHT - 170, 160, 160).collidepoint(game.get_game().displayer.reflect(*pg.mouse.get_pos())) and 1 in game.get_game().get_mouse_press():
                    rc = cur_recipe
                ...
                for i in range(-10, 10):
                    ...
                    styles.item_display(displayer.SCREEN_WIDTH - 90, displayer.SCREEN_HEIGHT // 2 + i * 80 - 40, cur_recipe.result, str(s + 1), str(cur_recipe.crafted_amount), 1)
                    i += 1
                cur_recipe = self.recipes[self.sel_recipe]
                styles.item_mouse(game.get_game().displayer.SCREEN_WIDTH - 260, game.get_game().displayer.SCREEN_HEIGHT - 170,
                                  cur_recipe.result, str(self.sel_recipe + 1), str(cur_recipe.crafted_amount), 2, anchor='right')
                i = 0
                for item, amount in cur_recipe.material.items():
                    styles.item_mouse(game.get_game().displayer.SCREEN_WIDTH - 20 - 80 * (1 + len(cur_recipe.material)) + 80 * i,
                                      game.get_game().displayer.SCREEN_HEIGHT - 250, item, '', str(amount), 1, anchor='right')
                    i += 1
                for i in range(-10, 10):
                    s = (self.sel_recipe + i + len(self.recipes)) % len(self.recipes)
                    cur_recipe = self.recipes[s]
                    styles.item_mouse(displayer.SCREEN_WIDTH - 90, displayer.SCREEN_HEIGHT // 2 + i * 80 - 40, cur_recipe.result, str(s + 1), str(cur_recipe.crafted_amount), 1, anchor='right')
                    r = pg.Rect(displayer.SCREEN_WIDTH - 90, displayer.SCREEN_HEIGHT // 2 + i * 80 - 40, 80, 80)
                    if r.collidepoint(game.get_game().displayer.reflect(*pg.mouse.get_pos())) and 1 in game.get_game().get_mouse_press():
                        self.sel_recipe = s
            ...
            displayer.canvas.blit(f, fr)
            pg.draw.rect(displayer.canvas, (0, 0, 0), (displayer.SCREEN_WIDTH - 120, displayer.SCREEN_HEIGHT - 110, 100, 100))
            ...
            for i in range(x - 10, x + 10):
                for j in range(y - 10, y + 10):
                    ...
                    pg.draw.rect(displayer.canvas, color, (displayer.SCREEN_WIDTH - 120 + (i - x + 10) * 5, displayer.SCREEN_HEIGHT - 110 + (j - y + 10) * 5, 5, 5))
        ...
        if pg.K_h in game.get_game().get_keys():
            potions = [inventory.ITEMS['butterscotch_pie'], inventory.ITEMS['crabapple'], inventory.ITEMS['weak_healing_potion']]
            for p in potions:
