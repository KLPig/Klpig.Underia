...
def item_mouse(x, y, name, no, amount, scale, anchor='left'):
    ...
    if pg.Rect(r).collidepoint(game.get_game().displayer.reflect(*pg.mouse.get_pos())):
        ...
        if anchor == 'left':
            tr = t.get_rect(bottomleft=(game.get_game().displayer.reflect(*pg.mouse.get_pos())[0], game.get_game().displayer.reflect(*pg.mouse.get_pos())[1] - 36 * l + p_y))
        else:
            tr = t.get_rect(bottomright=(game.get_game().displayer.reflect(*pg.mouse.get_pos())[0] - 80 * scale, game.get_game().displayer.reflect(*pg.mouse.get_pos())[1] - 36 * l + p_y))
        window.blit(t, tr)
        ...
        for j in range(l - 1):
            t = game.get_game().displayer.font.render(text(desc_split[j]), True,
                                   (255, 255, 255), (0, 0, 0))
            if anchor == 'left':
                tr = t.get_rect(bottomleft=(game.get_game().displayer.reflect(*pg.mouse.get_pos())[0], game.get_game().displayer.reflect(*pg.mouse.get_pos())[1] - 36 * (l - j - 1) + p_y))
            else:
                tr = t.get_rect(bottomright=(
                game.get_game().displayer.reflect(*pg.mouse.get_pos())[0] - 80 * scale, game.get_game().displayer.reflect(*pg.mouse.get_pos())[1] - 36 * (l - j - 1) + p_y))
            window.blit(t, tr)
