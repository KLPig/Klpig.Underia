...
class LIGHT:
    ...
    def baked_lighting(self, tiles, x, y, reset_surface):
        ...
        for point in self.get_tiles(tiles, x, y):
            ...
            self.fill_shadows(self.baked_pixel_shader_surf,
                              [corners[0], corners[1], self.get_intersection([self.radius] * 2, corners[1]),
                               self.get_intersection([self.radius] * 2, corners[0]), corners[2]])
        ...
        if points[2][0] + points[3][0] not in [1000, 0] and points[2][1] + points[3][1] not in [1000, 0]:
            if abs(points[2][0] - points[3][0]) == self.size:
                ...
                if self.radius < points[2][1]:
                    render_points = [points[0], points[4], points[1], points[2], [0, self.size], [self.size, self.size],
                                     points[3]]
                ...
                if self.radius < points[2][0]:
                    render_points = [points[0], points[4], points[1], points[2], [self.size, self.size], [self.size, 0],
                                     points[3]]
                ...
                if points[2][0] != self.size and points[2][0] != 0:
                    render_points = [points[0], points[4], points[1], points[2], [points[3][0], points[2][1]],
                                     points[3]]

                else:
                    render_points = [points[0], points[4], points[1], points[2], [points[2][0], points[3][1]],
                                     points[3]]
    ...
    def get_tiles(self, tiles, x, y):
        ...
        for rect in tiles:
            if (rect.x - x >= (-self.radius) - rect.width and rect.x - x <= self.radius) and (
                    rect.y - y >= (-self.radius) - rect.height and rect.y - y <= self.radius):
